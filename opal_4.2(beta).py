...
STARTUP_BACKUP_DIR = CONFIG_DIR / "startup_backups"
RESTART_MARKER_PATH = Path("restart_marker.json")
...
DEFAULT_HOLD_TIMEOUT_SECONDS = int(HOLD_TIMEOUT_DELTA.total_seconds())
MAX_HOLD_TIMEOUT_SECONDS = DEFAULT_HOLD_TIMEOUT_SECONDS
...
def normalize_config(raw: dict | None) -> dict:
    ...
    for key, record in config.get("punishments", {}).items():
        ...
        allow_duplicate_case_code = str(record.get("action")) == "언뮤트" and bool(case_code)
        if not case_code or (case_code.lower() in used_case_codes and not allow_duplicate_case_code):
    ...
    if config["default_timeout_seconds"] < 1:
        config["default_timeout_seconds"] = 600
    if not isinstance(config.get("hold_timeout_seconds"), int):
        config["hold_timeout_seconds"] = DEFAULT_HOLD_TIMEOUT_SECONDS
    if config["hold_timeout_seconds"] < 60 or config["hold_timeout_seconds"] > MAX_HOLD_TIMEOUT_SECONDS:
        config["hold_timeout_seconds"] = DEFAULT_HOLD_TIMEOUT_SECONDS
...
async def _db_prepare_for_restart() -> dict:
    async with aiosqlite.connect(DB_PATH) as db:
        await configure_db_connection(db)
        await db.commit()
        integrity_cursor = await db.execute("PRAGMA integrity_check")
        ...
        return {
            ...
            "checkpoint": tuple(checkpoint_row) if checkpoint_row else (),
        }
...
def merge_config_snapshots(json_config: dict | None, db_config: dict | None) -> dict:
    ...
    for key in ("default_timeout_seconds", "hold_timeout_seconds"):
        if key in db_config:
            merged[key] = db_norm[key]
...
def save_config(guild_id: int, config: dict) -> None:
    ensure_db_initialized()
    _run_db_coroutine(_db_save_config(guild_id, normalize_config(config)))
...
def hold_timeout_delta(config: dict | None) -> dt.timedelta:
    seconds = DEFAULT_HOLD_TIMEOUT_SECONDS
    if isinstance(config, dict):
        seconds = int(config.get("hold_timeout_seconds", DEFAULT_HOLD_TIMEOUT_SECONDS))
    seconds = min(max(60, seconds), MAX_HOLD_TIMEOUT_SECONDS)
...
def save_punishment_record(
    ...
    case_code = (case_code or make_case_code(config)).strip().lstrip("#")[:32]
...
def timeout_case_code(config: dict, timeout_data: dict | None) -> str | None:
    if not isinstance(timeout_data, dict):
        return None
    case_code = str(timeout_data.get("punishment_case_code") or "").strip().lstrip("#")
    if case_code:
        return case_code
    case_id = timeout_data.get("punishment_case_id")
    if case_id is not None:
        return get_case_code(config, case_id)
    return None
...
def load_restart_marker() -> dict | None:
    if not RESTART_MARKER_PATH.exists():
        return None
    try:
        return json.loads(RESTART_MARKER_PATH.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return None


def save_restart_marker(reason: str, backup_dir: Path | None) -> None:
    data = {
        "reason": reason,
        "backup_dir": str(backup_dir) if backup_dir is not None else None,
        "started_at": now_kst().isoformat(),
    }
    RESTART_MARKER_PATH.write_text(json.dumps(data, ensure_ascii=False, indent=4), encoding="utf-8")


async def send_restart_log_embed(title: str, description: str, color: int, timeout: float = 3.0) -> None:
    if not bot.guilds:
        return
    ...
    for guild in bot.guilds:
        channel_id = load_config(guild.id).get("log_channel")
        if not channel_id:
            continue
        ...
        tasks.append(asyncio.create_task(channel.send(embed=embed)))
...
def broadcast_restart_complete() -> None:
    restart_marker = load_restart_marker()
    if restart_marker is None:
        return

    reason = restart_marker.get("reason") or "알 수 없음"
    backup_dir = restart_marker.get("backup_dir")
    description = (
        f"이전 재시작이 완료되었습니다.\n사유: `{reason}`\n백업 폴더: `{Path(backup_dir or "없음").name}`"
    )

    async def _send_complete() -> None:
        await send_restart_log_embed("재시작 완료", description, GREEN, timeout=3.0)
        remove_restart_marker()

    try:
        bot.loop.create_task(_send_complete())
    except Exception:
        pass


def remove_restart_marker() -> None:
    try:
...
MAX_HTML_CELL_TEXT = 4000


def safe_html_text(value: object, limit: int = MAX_HTML_CELL_TEXT) -> str:
    text = str(value or "")
    if len(text) > limit:
...
def safe_attachment_url(url: str | None) -> str | None:
    if not url:
        return None
    try:
        parsed = urlparse(url)
    ...
    if not any(host == suffix.lstrip(".") or host.endswith(suffix) for suffix in SAFE_ATTACHMENT_HOST_SUFFIXES):
        return None
...
def purge_attachment_html(attachment: discord.Attachment) -> str:
    url = safe_attachment_url(getattr(attachment, "url", None))
    proxy_url = safe_attachment_url(getattr(attachment, "proxy_url", None))
    display_url = proxy_url or url
    filename = safe_html_text(attachment.filename or "attachment", 220)
    size_text = safe_html_text(format_file_size(getattr(attachment, "size", None)), 40)
    type_text = safe_html_text(getattr(attachment, "content_type", None) or Path(attachment.filename or "").suffix or "file", 80)

    if not url:
        return (
            '<li class="attachment blocked">'
            f'<span class="file-name">{filename}</span>'
            f'<span class="file-meta">{type_text} · {size_text} · blocked unsafe URL</span>'
            "</li>"
        )

    image_preview = ""
    if display_url and is_image_attachment(attachment):
        escaped_preview = html.escape(display_url, quote=True)
        image_preview = (
            '<a class="image-link" href="{0}" target="_blank" rel="noopener noreferrer">'
            '<img src="{0}" alt="{1}" loading="lazy" referrerpolicy="no-referrer">'
            "</a>"
        ).format(escaped_preview, filename)

    return (
        '<li class="attachment">'
        f"{image_preview}"
        '<div class="file-row">'
        f'<a href="{html.escape(url, quote=True)}" target="_blank" rel="noopener noreferrer">{filename}</a>'
        f'<span class="file-meta">{type_text} · {size_text}</span>'
        "</div>"
        "</li>"
    )
...
def purge_log_html(
    ...
    for message in reversed(messages):
        ...
        author = safe_html_text(f"{message.author} ({message.author.id})", 260)
        content = safe_html_text(message.clean_content or "(내용 없음)")
        attachments = "".join(purge_attachment_html(attachment) for attachment in message.attachments)
        ...
        rows.append(
            ...
            f"<div class=\"meta\"><span>{created_at}</span><span>{author}</span></div>"
            f"<div class=\"content\">{content}</div>"
            f"{attachment_block}{embed_text}"
            "</article>"
        )

    target_text = safe_html_text(str(target), 260) if target else "전체 메시지"
...
<meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src https://*.discordapp.com https://*.discordapp.net https://*.discord.com https://*.discordcdn.com https://*.discord.media; style-src 'unsafe-inline'; base-uri 'none'; form-action 'none'; object-src 'none'; frame-ancestors 'none'; navigate-to https://*.discordapp.com https://*.discordapp.net https://*.discord.com https://*.discordcdn.com https://*.discord.media;">
<meta name="referrer" content="no-referrer">
<title>Opal 청소 로그</title>
<style>
:root {{ color-scheme: dark; --bg: #101214; --panel: #171a1f; --line: #2b3038; --text: #edf0f3; --muted: #98a2b3; --accent: #65d6ad; --danger: #ff7a90; }}
* {{ box-sizing: border-box; }}
body {{ margin: 0; background: var(--bg); color: var(--text); font-family: "Segoe UI", Arial, sans-serif; }}
main {{ width: min(1080px, calc(100% - 32px)); margin: 28px auto 48px; }}
header {{ border-bottom: 1px solid var(--line); padding-bottom: 18px; margin-bottom: 18px; }}
h1 {{ font-size: 24px; line-height: 1.2; margin: 0 0 12px; letter-spacing: 0; }}
.summary {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(190px, 1fr)); gap: 8px; color: var(--muted); font-size: 13px; }}
.summary b {{ display: block; color: var(--text); font-size: 15px; margin-top: 2px; overflow-wrap: anywhere; }}
.message {{ background: var(--panel); border: 1px solid var(--line); border-radius: 8px; padding: 14px; margin: 10px 0; }}
.meta {{ display: flex; gap: 10px; flex-wrap: wrap; color: var(--muted); font-size: 12px; margin-bottom: 10px; }}
.content {{ white-space: pre-wrap; line-height: 1.5; overflow-wrap: anywhere; }}
.attachments {{ list-style: none; padding: 0; margin: 12px 0 0; display: grid; gap: 10px; }}
.attachment {{ border: 1px solid var(--line); border-radius: 8px; overflow: hidden; background: #111418; }}
.attachment.blocked {{ padding: 10px; border-color: rgba(255, 122, 144, .55); }}
.image-link {{ display: block; background: #0b0d10; }}
img {{ display: block; max-width: 100%; max-height: 520px; object-fit: contain; }}
.file-row {{ display: flex; gap: 10px; align-items: center; justify-content: space-between; padding: 10px; flex-wrap: wrap; }}
...
<div>서버<b>{safe_html_text(guild.name, 260)}</b></div>
<div>채널<b>#{safe_html_text(channel.name, 180)}</b></div>
<div>처리자<b>{safe_html_text(str(moderator), 260)}</b></div>
<div>대상<b>{target_text}</b></div>
<div>삭제<b>{len(messages)}개</b></div>
<div>생성<b>{generated_at}</b></div>
</section>
</header>
{''.join(rows) if rows else '<article class="message"><div class="content">삭제된 메시지가 없습니다.</div></article>'}
</main>
</body>
</html>"""
...
async def safely_delete_messages(messages: list[discord.Message]) -> list[discord.Message]:
...
def latest_timeout_record(config: dict, target_id: int) -> tuple[str | None, dict | None]:
    ...
    for key, record in config.get("punishments", {}).items():
        ...
        if int(record.get("target_id", 0)) != target_id:
            continue
        if not str(record.get("action", "")).startswith("뮤트"):
            continue
...
async def log_timeout_expired(guild: discord.Guild, member: discord.Member, config: dict, timeout_data: dict | None = None) -> None:
    case_id = next_case_id(config)
    release_case_code = timeout_case_code(config, timeout_data)
    save_punishment_record(config, case_id, "언뮤트", member, guild.me, "Expired", release_case_code)
    save_config(guild.id, config)
...
async def renew_hold_timeout(guild: discord.Guild, member: discord.Member, config: dict, timeout_data: dict) -> None:
    until = dt.datetime.now(dt.timezone.utc) + hold_timeout_delta(config)
...
def backup_sqlite_configs_once(force: bool = False, strict: bool = False) -> Path | None:
    """SQLite 설정을 JSON 백업본으로 내보냅니다. 기본값은 봇 시작 시 한 번만 실행합니다."""
    global STARTUP_BACKUP_DONE
    if STARTUP_BACKUP_DONE and not force:
        return None
    STARTUP_BACKUP_DONE = True
    ensure_db_initialized()
    try:
        guild_ids = _run_db_coroutine(_db_list_guild_ids())
    except Exception as exc:
        print(f"시작 백업 대상 로드 실패: {exc}")
        if strict:
            raise
        return None
    ...
    STARTUP_BACKUP_DIR.mkdir(parents=True, exist_ok=True)
    stamp = now_kst().strftime("%Y%m%d-%H%M%S")
    backup_dir = STARTUP_BACKUP_DIR / f"{stamp}-{os.getpid()}"
    backup_dir.mkdir(parents=True, exist_ok=False)

    saved = 0
    for guild_id in guild_ids:
        try:
            config = _run_db_coroutine(_db_load_config(int(guild_id)))
            if isinstance(config, dict):
                data = json.dumps(normalize_config(config), ensure_ascii=False, indent=4)
                backup_path = backup_dir / f"{guild_id}.json"
                tmp_path = backup_dir / f".{guild_id}.{os.getpid()}.{secrets.token_hex(4)}.tmp"
                tmp_path.write_text(data, encoding="utf-8")
                os.replace(tmp_path, backup_path)
                json.loads(backup_path.read_text(encoding="utf-8"))
                saved += 1
        except Exception as exc:
            print(f"시작 백업 실패 ({guild_id}): {exc}")
            if strict:
                raise

    if strict and saved != len(guild_ids):
        raise RuntimeError(f"백업 검증 실패: {len(guild_ids)}개 중 {saved}개 저장")
    print(f"시작 백업 완료: {saved}개 JSON 저장 ({backup_dir})")
    return backup_dir


async def sync_slash_commands(force: bool = False) -> None:
    """슬래시 명령어를 전역으로 한 번만 동기화합니다."""
    global SLASH_SYNCED_ON_READY
    if SLASH_SYNCED_ON_READY and not force:
        return
    try:
...
async def save_data_and_restart(reason: str) -> None:
    global RESTART_IN_PROGRESS
    if RESTART_IN_PROGRESS:
        raise RuntimeError("이미 재시작이 진행 중입니다.")
    RESTART_IN_PROGRESS = True
    print(f"저장 후 재시작 시작: {reason}")

    def prepare_restart() -> Path | None:
        ensure_db_initialized()
        db_state = _run_db_coroutine(_db_prepare_for_restart(), timeout=30)
        if db_state.get("integrity") != "ok":
            raise RuntimeError(f"SQLite integrity_check 실패: {db_state.get('integrity')}")
        print(f"SQLite 재시작 준비 완료: checkpoint={db_state.get('checkpoint')}")
        return backup_sqlite_configs_once(force=True, strict=True)

    try:
        backup_dir = await bot.loop.run_in_executor(None, prepare_restart)

        if backup_dir:
            print(f"재시작 전 백업 완료: {backup_dir}")

        save_restart_marker(reason, backup_dir)
        try:
            bot.loop.create_task(send_restart_log_embed(
                "재시작 중",
                f"{reason} 사유로 재시작을 진행하고 있어요.\n백업 폴더: `{backup_dir}`",
                YELLOW,
            ))
        except Exception:
            pass
        ...
        print("restart executable =", python_executable)
        print("restart args =", restart_args)

        if platform.system() == "Windows":
            subprocess.Popen(restart_args, cwd=os.getcwd(), env=os.environ)
            sys.exit(0)
...
async def weekly_save_and_restart() -> None:
    try:
        await save_data_and_restart("weekly")
    except Exception as exc:
        print(f"주간 재시작 실패: {exc}")


@weekly_save_and_restart.before_loop
async def before_weekly_save_and_restart() -> None:
    await bot.wait_until_ready()
...
async def setup_hook() -> None:
    ...
    if not weekly_save_and_restart.is_running():
        weekly_save_and_restart.start()
...
async def on_member_update(before: discord.Member, after: discord.Member) -> None:
    ...
    if after_timeout is not None and (
        ...
        punishment_case_code = existing_timeout.get("punishment_case_code") if isinstance(existing_timeout, dict) else None
        config["active_timeouts"][str(after.id)] = {
            ...
            "punishment_case_code": punishment_case_code,
        ...
        if case_id:
            ...
            config["active_timeouts"][str(after.id)]["punishment_case_code"] = get_case_code(config, case_id)
    ...
    config = load_config(after.guild.id)
    case_id = next_case_id(config)
    release_case_code = timeout_case_code(config, timeout_data if isinstance(timeout_data, dict) else None)
    save_punishment_record(config, case_id, "언뮤트", after, moderator, release_reason, release_case_code)
    save_config(after.guild.id, config)
...
async def sync_commands_command(ctx: commands.Context) -> None:
    ...
    await sync_slash_commands(force=True)
    await message.edit(embed=make_embed("동기화 완료", "잠시 뒤 Discord 명령어 입력창에서 다시 확인해 주세요.", GREEN))


@bot.command(name="재시작", aliases=["restart", "리부트", "reboot"])
async def restart_command(ctx: commands.Context) -> None:
    if not await is_developer(ctx.author):
        await ctx.reply(embed=make_embed("권한 부족", "봇 재시작은 개발자만 실행할 수 있어요.", RED), mention_author=False)
        return
    if RESTART_IN_PROGRESS:
        await ctx.reply(embed=make_embed("재시작 진행 중", "이미 재시작 절차가 진행 중이에요.", YELLOW), mention_author=False)
        return

    message = await ctx.reply(embed=make_embed("재시작 준비 중", "DB 점검과 백업을 먼저 진행할게요.", YELLOW), mention_author=False)
    try:
        await save_data_and_restart(f"manual:{ctx.author.id}")
    except Exception as exc:
        await message.edit(embed=make_embed("재시작 실패", f"안전 점검 중 문제가 생겨 재시작을 중단했어요.\n`{short_reason(str(exc))}`", RED))
...
@bot.tree.command(name="hold", description="hold 타임아웃이 갱신될 기간을 설정합니다.")
@app_commands.default_permissions(manage_guild=True)
async def slash_set_hold_timeout(interaction: discord.Interaction, 기간: str) -> None:
    if not await require_manage_guild_interaction(interaction):
        return

    delta = parse_duration(기간)
    if not delta:
        embed = make_embed("시간 형식 오류", "예: `10분`, `2시간`, `1일`, `1주`, `하루`, `일주일`, `10m`, `2h`, `1w`", RED)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    seconds = int(delta.total_seconds())
    if seconds < 60:
        embed = make_embed("시간 초과", "hold 기간은 최소 `1분` 이상이어야 해요.", RED)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    if seconds > MAX_HOLD_TIMEOUT_SECONDS:
        embed = make_embed("시간 초과", f"hold 기간은 최대 `{seconds_to_duration_text(MAX_HOLD_TIMEOUT_SECONDS)}`까지 가능해요.", RED)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return

    config = load_config(interaction.guild.id)
    config["hold_timeout_seconds"] = seconds
    save_config(interaction.guild.id, config)
    embed = make_embed("hold 기간 설정 완료", f"hold 타임아웃 갱신 기간을 `{seconds_to_duration_text(seconds)}`로 설정했어요.", GREEN)
    await interaction.response.send_message(embed=embed, ephemeral=True)
        ...
        try:
            if action == "timeout":
                ...
                config["active_timeouts"][str(member.id)] = {
                    ...
                    "punishment_case_code": get_case_code(config, timeout_case_id) if timeout_case_id else None,
...
async def edit_punishment(ctx: commands.Context, case_id: str, *, changes: str = "") -> None:
    ...
    if new_duration_text and not old_action.startswith("뮤트"):
        await ctx.reply(embed=make_embed("수정 실패", "타임아웃 시간 수정은 타임아웃 처벌 기록에서만 가능해요.", RED), mention_author=False)
        return

    if new_duration_text and old_action.startswith("뮤트"):
        ...
        if member:
            ...
            active_config["active_timeouts"][str(member.id)] = {
                ...
                "punishment_case_code": get_case_code(active_config, clean_case_id),
...
async def mute(ctx: commands.Context, target: str, duration: str | None = None, *, reason: str | None = None) -> None:
    ...
    delta = hold_timeout_delta(config) if hold_timeout else parse_duration(duration_text) if duration_text else None
    if delta:
...
async def unmute(ctx: commands.Context, target: str, *, reason: str | None = None) -> None:
    ...
    try:
        ...
        case_id = next_case_id(config)
        release_case_code = timeout_case_code(config, timeout_data if isinstance(timeout_data, dict) else None)
        save_punishment_record(config, case_id, "언뮤트", member, ctx.author, release_reason, release_case_code)
        save_config(ctx.guild.id, config)
...
async def purge(ctx: commands.Context, *args: str) -> None:
    ...
    skip_ids = {ctx.message.id, processing.id}

    try:
        async for message in ctx.channel.history(limit=scan_limit, before=processing):
        ...
        await processing.edit(embed=make_embed("청소 실패", "메시지 조회 중 Discord 오류가 발생했어요.", RED))
        return

    if not to_delete:
        await processing.edit(embed=make_embed("청소 완료", "삭제할 메시지가 없어요.", YELLOW))
        await processing.delete(delay=5)
        return

    try:
        deleted = await safely_delete_messages(to_delete)
    except discord.Forbidden:
