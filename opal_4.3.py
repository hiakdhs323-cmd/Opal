...
SLASH_GUILD_COMMANDS_CLEARED_PATH = Path("slash_guild_commands_cleared.json")
BULK_DELETE_CHUNK_SIZE = 100
PURGE_MAX_MESSAGES = 1000
PURGE_SCAN_LIMIT_MAX = 5000
...
SLASH_GUILD_COMMANDS_CLEARED = SLASH_GUILD_COMMANDS_CLEARED_PATH.exists()
SLASH_SYNC_LOCK = asyncio.Lock()
HOLD_TIMEOUT_DELTA = dt.timedelta(days=27, hours=23)
HOLD_TIMEOUT_TEXT = "pending"
RESTART_LOG_HANDLED_ON_READY = False
...
def normalize_config(raw: dict | None) -> dict:
    ...
    for key, record in config.get("punishments", {}).items():
        ...
        allow_duplicate_case_code = bool(record.get("allow_duplicate_case_code"))
...
async def fetch_parent_ban_case_code(guild_id: int, target_id: int) -> str | None:
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            await configure_db_connection(db)
            cursor = await db.execute(
                "SELECT case_code FROM case_logs WHERE guild_id = ? AND target_id = ? AND action = '차단' ORDER BY id DESC LIMIT 1",
                (guild_id, target_id),
            )
            row = await cursor.fetchone()
            return str(row[0]).lstrip("#") if row and row[0] else None
    except aiosqlite.Error as exc:
        print(f"이전 차단 사건 코드 조회 실패: {exc}")
        return None
...
def restart_complete_embed(marker: dict) -> discord.Embed:
    requested_by = marker.get("requested_by")
    requested_at = marker.get("requested_at")
    description = "봇 재부팅이 완료되어 다시 온라인 상태가 되었어요."
    if requested_by:
        description += f"\n요청자: <@{requested_by}> (`{requested_by}`)"
    if requested_at:
        description += f"\n요청 시각: `{requested_at}`"

    embed = make_embed("재부팅 완료", description, GREEN)
    embed.add_field(name="현재 PID", value=f"`{os.getpid()}`", inline=True)
    old_pid = marker.get("pid")
    if old_pid:
        embed.add_field(name="이전 PID", value=f"`{old_pid}`", inline=True)
    return embed


async def handle_restart_marker_on_ready() -> None:
    if not RESTART_MARKER_PATH.exists():
        return

    try:
        marker = json.loads(RESTART_MARKER_PATH.read_text(encoding="utf-8"))
        channel_id = int(marker.get("channel_id", 0))
        channel = bot.get_channel(channel_id)
        if channel is None:
            print(f"재부팅 완료 로그 채널을 찾을 수 없음: {channel_id}")
            return
        if not hasattr(channel, "send"):
            print(f"재부팅 완료 로그를 보낼 수 없는 채널: {channel_id}")
            return
        await channel.send(embed=restart_complete_embed(marker))
    except (OSError, json.JSONDecodeError, TypeError, ValueError) as exc:
        print(f"재부팅 마커 처리 실패: {exc}")
    except discord.DiscordException as exc:
        print(f"재부팅 완료 로그 전송 실패: {exc}")
    finally:
        try:
...
def save_punishment_record(
    ...
    allow_duplicate_case_code: bool = False,
) -> None:
    case_code = str(case_code).strip().lstrip("#")[:32] if case_code else make_case_code(config)
    config["punishments"][str(case_id)] = {
        "case_id": case_id,
        "case_code": case_code,
        "allow_duplicate_case_code": allow_duplicate_case_code,
        "action": action,
        "target_id": target.id,
...
async def safely_delete_messages(messages: list[discord.Message]) -> list[discord.Message]:
    ...
    channel = unique_messages[0].channel
    if any(message.channel.id != channel.id for message in unique_messages):
        raise ValueError("한 번의 대량 삭제에는 같은 채널의 메시지만 전달할 수 있습니다.")
    if not hasattr(channel, "delete_messages"):
        raise ValueError("이 채널은 대량 삭제를 지원하지 않습니다.")

    bulk_cutoff = dt.datetime.now(dt.timezone.utc) - dt.timedelta(days=14)
    bulk_safe_messages = [
        message
        for message in unique_messages
        if message.created_at.astimezone(dt.timezone.utc) > bulk_cutoff
    ]
    deleted_messages = []
    for index in range(0, len(bulk_safe_messages), BULK_DELETE_CHUNK_SIZE):
        chunk = bulk_safe_messages[index:index + BULK_DELETE_CHUNK_SIZE]
        if not chunk:
            continue
        await channel.delete_messages(chunk)
        deleted_messages.extend(chunk)
...
def purge_log_html(
    ...
    def render_custom_emojis(value: str) -> str:
        escaped = html.escape(value or "(내용 없음)")
        ...
        def repl(match: re.Match[str]) -> str:
            ...
            return (
                f'<img class="emoji" src="{url}" alt=":{safe_name}:" '
                f'title=":{safe_name}:">:{safe_name}:'
            )

        return re.sub(r"&lt;(a?):([A-Za-z0-9_]{2,32}):(\d{15,25})&gt;", repl, escaped)

    def render_attachment(attachment: discord.Attachment) -> str:
        filename = html.escape(attachment.filename)
        url = html.escape(attachment.url)
        content_type = (attachment.content_type or "").lower()
        if content_type.startswith("image/"):
            return f'<li><a href="{url}">{filename}</a><br><img class="media" src="{url}" alt="{filename}"></li>'
        if content_type.startswith("video/"):
            return f'<li><a href="{url}">{filename}</a><br><video class="media" controls src="{url}"></video></li>'
        return f'<li><a href="{url}">{filename}</a></li>'

    def render_sticker(sticker: discord.StickerItem) -> str:
        name = html.escape(sticker.name)
        sticker_url = getattr(sticker, "url", None)
        if sticker_url:
            safe_url = html.escape(sticker_url)
            return f'<li><a href="{safe_url}">{name}</a><br><img class="sticker" src="{safe_url}" alt="{name}"></li>'
        return f"<li>{name}</li>"

    def embed_summary(embed: discord.Embed) -> str:
        data = embed.to_dict()
        parts = []
        for key in ("title", "description", "url"):
            value = data.get(key)
            if value:
                parts.append(f"<div><b>{html.escape(key)}</b>: {render_custom_emojis(str(value))}</div>")
        for image_key in ("thumbnail", "image"):
            image_url = data.get(image_key, {}).get("url") if isinstance(data.get(image_key), dict) else None
            if image_url:
                parts.append(
                    f'<div><b>{html.escape(image_key)}</b><br>'
                    f'<img class="media" src="{html.escape(str(image_url))}" alt="{html.escape(image_key)}"></div>'
                )
        fields = data.get("fields") or []
        for field in fields[:12]:
            if not isinstance(field, dict):
                continue
            name = html.escape(str(field.get("name", "field")))
            value = render_custom_emojis(str(field.get("value", "")))
            parts.append(f"<div><b>{name}</b>: {value}</div>")
        return "".join(parts) or "<div>임베드 내용 없음</div>"
    ...
    for message in reversed(messages):
        ...
        content = render_custom_emojis(message.clean_content or message.content or "(내용 없음)")
        attachments = "".join(
            render_attachment(attachment)
            for attachment in message.attachments
        )
        attachment_block = f"<ul class=\"media-list\">{attachments}</ul>" if attachments else ""
        stickers = "".join(render_sticker(sticker) for sticker in getattr(message, "stickers", []))
        sticker_block = f"<div class=\"block-title\">스티커</div><ul class=\"media-list\">{stickers}</ul>" if stickers else ""
        embed_blocks = "".join(f"<section class=\"embed-box\">{embed_summary(embed)}</section>" for embed in message.embeds)
        embed_text = f"<div class=\"block-title\">임베드 {len(message.embeds)}개</div>{embed_blocks}" if message.embeds else ""
        rows.append(
            "<article>"
            f"<div class=\"row-head\"><img class=\"avatar\" src=\"{avatar}\" alt=\"\"><div><div class=\"author\">{author}</div><div class=\"meta\">{created_at}</div></div></div>"
            f"<div class=\"content\">{content}</div>"
            f"{attachment_block}{sticker_block}{embed_text}"
            "</article>"
        )
...
<style>
body {{ font-family: "Segoe UI", Arial, sans-serif; max-width: 980px; margin: 32px auto; padding: 0 16px; color: #202225; background: #f5f6f8; }}
h1 {{ font-size: 24px; margin-bottom: 6px; }}
.summary {{ color: #4b5563; margin-bottom: 18px; padding: 14px 16px; background: #fff; border: 1px solid #d8dce3; border-radius: 8px; }}
article {{ background: #fff; border: 1px solid #d8dce3; border-radius: 8px; padding: 14px; margin: 10px 0; }}
.row-head {{ display: flex; gap: 10px; align-items: center; margin-bottom: 10px; }}
.avatar {{ width: 38px; height: 38px; border-radius: 50%; }}
.author {{ font-weight: 700; }}
.meta {{ color: #667085; font-size: 13px; }}
.content {{ white-space: pre-wrap; line-height: 1.5; margin: 8px 0; }}
.media-list {{ margin: 8px 0; padding-left: 22px; }}
.media {{ display: block; max-width: min(720px, 100%); max-height: 420px; margin-top: 6px; border-radius: 6px; border: 1px solid #e5e7eb; }}
.sticker {{ max-width: 180px; max-height: 180px; }}
.emoji {{ width: 22px; height: 22px; vertical-align: -5px; margin: 0 2px; }}
.block-title {{ margin-top: 10px; color: #475467; font-size: 13px; font-weight: 700; }}
.embed-box {{ border-left: 4px solid #5865f2; background: #f8fafc; padding: 10px 12px; margin-top: 8px; border-radius: 6px; }}
...
async def send_purge_log(
    ...
    try:
        await channel.send(embed=embed, file=discord.File(file_path), allowed_mentions=discord.AllowedMentions.none())
    except discord.DiscordException:
        pass
    finally:
        try:
...
def duplicate_case_code(record: dict | None) -> str | None:
    if isinstance(record, dict) and record.get("case_code"):
        return str(record["case_code"]).lstrip("#")
    return None


def timeout_parent_case_code(config: dict, target_id: int, timeout_data: dict | None = None) -> str | None:
    if isinstance(timeout_data, dict):
        punishment_case_id = timeout_data.get("punishment_case_id")
        if punishment_case_id:
            return get_case_code(config, punishment_case_id).lstrip("#")

    _, timeout_record = latest_timeout_record(config, target_id)
    return duplicate_case_code(timeout_record)
...
async def log_timeout_expired(guild: discord.Guild, member: discord.Member, config: dict, timeout_data: dict | None = None) -> None:
    case_id = next_case_id(config)
    parent_code = timeout_parent_case_code(config, member.id, timeout_data)
    save_punishment_record(
        config,
        case_id,
        "언뮤트",
        ...
        guild.me,
        "Expired",
        case_code=parent_code,
        allow_duplicate_case_code=bool(parent_code),
    )
    save_config(guild.id, config)
...
async def sync_slash_commands() -> None:
    """슬래시 명령어를 동기화합니다."""
    global SLASH_GUILD_COMMANDS_CLEARED
    async with SLASH_SYNC_LOCK:
        try:
            global_synced = await bot.tree.sync()
            print(f"전역 슬래시 명령어 동기화 완료: {len(global_synced)}개")

            if not SLASH_GUILD_COMMANDS_CLEARED:
                cleanup_failed = False
                for guild in bot.guilds:
                    try:
                        bot.tree.clear_commands(guild=guild)
                        await bot.tree.sync(guild=guild)
                        print(f"서버 슬래시 명령어 중복 정리 완료: {guild.name}({guild.id})")
                    except Exception as guild_exc:
                        cleanup_failed = True
                        print(f"서버 슬래시 명령어 중복 정리 실패: {guild.name}({guild.id}) {guild_exc}")
                if not cleanup_failed:
                    SLASH_GUILD_COMMANDS_CLEARED = True
                    try:
                        SLASH_GUILD_COMMANDS_CLEARED_PATH.write_text(
                            json.dumps({"cleared_at": now_kst().isoformat()}, ensure_ascii=False, indent=2),
                            encoding="utf-8",
                        )
                    except OSError as marker_exc:
                        print(f"슬래시 중복 정리 마커 저장 실패: {marker_exc}")
        except Exception as exc:
...
async def on_ready() -> None:
    global SCHEDULED_TIMEOUTS_ON_READY, SLASH_SYNCED_ON_READY, RESTART_LOG_HANDLED_ON_READY
    if not RESTART_LOG_HANDLED_ON_READY:
        RESTART_LOG_HANDLED_ON_READY = True
        await handle_restart_marker_on_ready()
...
async def on_member_update(before: discord.Member, after: discord.Member) -> None:
    ...
    config = load_config(after.guild.id)
    case_id = next_case_id(config)
    parent_code = timeout_parent_case_code(config, after.id, timeout_data if isinstance(timeout_data, dict) else None)
    save_punishment_record(
        config,
        case_id,
        "언뮤트",
...
async def on_member_unban(guild: discord.Guild, user: discord.User) -> None:
    ...
    parent_code = await fetch_parent_ban_case_code(guild.id, user.id)
    if not parent_code:
        parent_code = duplicate_case_code(ban_record)
    case_id = await log_external_punishment(
        guild,
        "언밴",
        user,
        moderator,
...
async def restart_command(ctx: commands.Context) -> None:
    if not await is_developer(ctx.author):
        await ctx.reply(embed=make_embed("권한 부족", "봇 개발자만 재시작할 수 있어요.", RED), mention_author=False)
        return
    ...
    try:
        RESTART_MARKER_PATH.write_text(json.dumps(marker, ensure_ascii=False, indent=2), encoding="utf-8")
    except OSError as exc:
        await ctx.reply(embed=make_embed("재시작 실패", f"재부팅 마커를 저장하지 못했어요: `{exc}`", RED), mention_author=False)
        return

    await ctx.reply(embed=make_embed("재시작 중", "프로세스를 다시 시작하고 있어요. 완료되면 이 채널에 알려드릴게요.", YELLOW), mention_author=False)
    ...
    async def restart_process() -> None:
        ...
        try:
            ...
            try:
                ...
                print(f"프로세스 재시작 실패: {exc}")

    bot.loop.create_task(restart_process())


@bot.command(name="종료", aliases=["shutdown", "끄기", "봇종료", "전체종료", "stop"])
async def shutdown_command(ctx: commands.Context) -> None:
    if not await is_developer(ctx.author):
        await ctx.reply(embed=make_embed("권한 부족", "봇 개발자만 종료할 수 있어요.", RED), mention_author=False)
        return

    await ctx.reply(embed=make_embed("종료 중", "봇 프로세스를 안전하게 종료하고 있어요.", YELLOW), mention_author=False)

    async def shutdown_process() -> None:
        await asyncio.sleep(1)
        await bot.close()

    bot.loop.create_task(shutdown_process())
...
async def slash_delete_warning(interaction: discord.Interaction, 처벌코드: str, 유저: discord.Member | None = None, 사유: str | None = None) -> None:
    ...
    save_punishment_record(
        ...
        case_id,
        "경고 삭제",
        member or discord.Object(id=int(user_id)),
        interaction.user,
        사유,
        case_code=str(removed_code),
        allow_duplicate_case_code=True,
    )
    save_config(interaction.guild.id, config)
...
async def slash_clear_warnings(interaction: discord.Interaction, 유저: discord.Member, 사유: str | None = None) -> None:
    ...
    config = load_config(interaction.guild.id)
    removed_warnings = list(config["warnings"].get(str(유저.id), []))
    removed = len(removed_warnings)
    if removed <= 0:
    ...
    for warning in removed_warnings:
        removed_code = warning.get("case_code") or warning.get("case_id")
        case_id = next_case_id(config)
        save_punishment_record(
            config,
            case_id,
            ...
            interaction.user,
            사유,
            case_code=str(removed_code) if removed_code else None,
            allow_duplicate_case_code=bool(removed_code),
        )
    save_config(interaction.guild.id, config)
...
async def clear_warnings(ctx: commands.Context, target: str, *, reason: str | None = None) -> None:
    ...
    config = load_config(ctx.guild.id)
    removed_warnings = list(config["warnings"].get(str(member.id), []))
    removed = len(removed_warnings)
    if removed <= 0:
...
async def unmute(ctx: commands.Context, target: str, *, reason: str | None = None) -> None:
    ...
    try:
        ...
        case_id = next_case_id(config)
        parent_code = timeout_parent_case_code(config, member.id, timeout_data if isinstance(timeout_data, dict) else None)
        save_punishment_record(
            config,
            case_id,
            "언뮤트",
...
async def unban(ctx: commands.Context, user_id: int, *, reason: str | None = None) -> None:
    ...
    try:
        ...
        await ctx.reply(embed=make_embed("언밴 실패", "봇에게 차단 목록을 조회할 권한이 부족해요.", RED), mention_author=False)
        return
    except discord.DiscordException:
        await ctx.reply(embed=make_embed("언밴 실패", "Discord 차단 목록 조회 중 오류가 발생했어요.", RED), mention_author=False)
        return
    ...
    parent_code = await fetch_parent_ban_case_code(ctx.guild.id, user_id)
    if not parent_code and isinstance(ban_record, dict) and ban_record.get("case_code"):
        parent_code = str(ban_record["case_code"]).lstrip("#")
    ...
    try:
        ...
        await ctx.reply(embed=make_embed("언밴 실패", "봇에게 차단 해제 권한이 부족해요.", RED), mention_author=False)
        return
    except discord.DiscordException:
        await ctx.reply(embed=make_embed("언밴 실패", "Discord 차단 해제 중 오류가 발생했어요.", RED), mention_author=False)
        return
...
async def purge(ctx: commands.Context, *args: str) -> None:
    ...
    if amount < 1 or amount > PURGE_MAX_MESSAGES:
        await ctx.reply(embed=make_embed("범위 오류", f"1개부터 {PURGE_MAX_MESSAGES}개까지 삭제할 수 있어요.", RED), mention_author=False)
        return

    scan_limit = amount if target_member is None else min(max(amount * 8, 100), PURGE_SCAN_LIMIT_MAX)
    ...
    try:
        ...
        async for message in ctx.channel.history(limit=scan_limit, before=processing):
            if should_delete(message):
                delete_targets.append(message)
                if len(delete_targets) >= amount:
                    break
        deleted = await safely_delete_messages(delete_targets)
    except discord.Forbidden:
        await processing.edit(embed=make_embed("청소 실패", "봇에게 메시지 관리 권한이 부족해요.", RED))
        return
    except ValueError:
        await processing.edit(embed=make_embed("청소 실패", "같은 채널의 메시지만 한 번에 삭제할 수 있어요.", RED))
        return
    except discord.DiscordException:
